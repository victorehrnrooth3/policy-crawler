...
policy-crawler/
  ...
  src/policy_crawler/
    ...
    obs/
      cost.py               # price table + daily/monthly/per-run spend + degrade check
      alerts.py             # failure / warning emails (deduped per run per day)
...
- **Anthropic prices live only in `obs/cost.py`**: `PRICES_USD_PER_M_TOKENS` is the single source of truth, keyed by model-id *prefix* so dated ids (`claude-haiku-4-5-20251001`) resolve to the base entry. Per-module `_cost` helpers (e.g. `ranker/pass2.py`) delegate to `compute_call_cost`; never hard-code a second copy of the prices. When prices change, edit this one table.
- **`runs.total_cost_usd` is re-derived, not threaded**: the `run.py` wrapper sums all `llm_calls` for the run at finish time (`obs.cost.run_spend`), which is the only way crawl_extract cost gets folded in — the crawl summary doesn't carry it. If you add a new LLM call site, just log an `llm_calls` row with the `run_id` and the cost shows up automatically.
- **Cost caps: soft degrades, hard aborts**: `DAILY_SOFT_CAP_USD` (default $0.30) flips Pass 2 to Haiku (`should_degrade_to_haiku`, checked once in `score_pending` before the batch); `HARD_KILL_USD` (default $2.00) is the only real abort and merely skips the expensive Pass 2. Both log; degradation also tags `llm_calls.metadata.degraded`. Never make the soft cap abort.
- **Alerts dedupe per run per day**: `obs/alerts.py` writes `runs.metadata.alert_sent_at`; a second failure/warning for the same run that day is dropped. Both alert functions are best-effort and never raise — an alert failure must not mask the original error.
- **Webapp job state ≠ feedback signal**: `jobs.saved_at`/`archived_at`/`deleted_at` are *view state* for the inbox tabs and are queried directly (don't re-derive them from `feedback` votes). **Delete is signal-free** — `/inbox/action` op=`delete` sets `deleted_at` only and writes NO `feedback` row, so the agent isn't taught from decluttering. Only up/down/save log a `feedback` vote. The five views (`inbox`/`recommended`/`saved`/`all`/`archived`) all exclude `deleted_at IS NOT NULL`; all but `archived` also exclude `archived_at IS NOT NULL`.
